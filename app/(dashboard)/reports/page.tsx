...
export default async function ReportsPage() {
  ...
  const [paidResult, allResult, dailyResult, topProductsResult, marginResult] = await Promise.all([
    ...
    admin.from('invoice_lines').select('quantity, unit_price').gte('created_at', thirtyDaysAgoStr),
  ]);
  ...
  const productQuantityMap = new Map<string, { qty: number; revenue: number; name?: string }>();
  ...
  const totalLinesCost = (marginResult.data ?? []).reduce((sum, line) => sum + Number(line.quantity) * 0, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 lg:gap-4">
        ...
        <Card>
          <CardHeader className="px-5 lg:px-6 py-4 lg:py-5">
            <CardTitle className="text-sm font-heading font-semibold text-zinc-500">Factures par statut</CardTitle>
          </CardHeader>
          <CardContent className="px-5 lg:px-6 pb-5 lg:pb-6">
