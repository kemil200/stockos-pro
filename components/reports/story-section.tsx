...
import { TrendingUp, TrendingDown, Star, AlertTriangle, DollarSign } from 'lucide-react';
...
export function StorySection({ revenue, costs, cogs, expenses, profit, marginPct, nbSales, avgBasket, topProduct, lowStockCount, label }: StoryProps) {
  if (nbSales === 0) {
    return (
  ...
  return (
    <div className="bg-white rounded-2xl border border-zinc-200/80 p-5 sm:p-6 space-y-5 text-sm leading-relaxed">
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          <p className="text-zinc-800">
            Vos dépenses s&apos;élèvent à <strong className="text-red-500">{formatCurrency(costs)}</strong>
            {cogs > 0 && expenses > 0 ? (
              <> (dont <strong>{formatCurrency(cogs)}</strong> de marchandises vendues et <strong>{formatCurrency(expenses)}</strong> de charges).</>
            ) : cogs > 0 ? (
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          {isProfitable ? (
            <p className="text-zinc-800">
              Votre <strong>bénéfice</strong> est de <strong className="text-emerald-600">{formatCurrency(profit)}</strong>.
              {marginGood ? (
                <> C&apos;est une <strong className="text-emerald-600">bonne performance</strong> : pour chaque 100 FCFA vendus, vous gagnez <strong>{marginPct.toFixed(0)} FCFA</strong>.</>
              ) : (
                <> Pour chaque 100 FCFA vendus, il vous reste <strong>{marginPct.toFixed(0)} FCFA</strong>. {marginPct < 10 ? 'Essayez de négocier vos prix d\'achat pour améliorer votre marge.' : ''}</>
              )}
            </p>
          ) : (
            <p className="text-zinc-800">
              Vos ventes ne couvrent pas vos dépenses. <strong className="text-red-500">Perte de {formatCurrency(Math.abs(profit))}</strong>.
              Vérifiez vos prix de vente ou réduisez vos charges.
            </p>
          )}
        </div>
      </div>
      ...
      {topProduct && (
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-zinc-800">
              Votre <strong>produit star</strong> est <strong className="text-blue-600">{topProduct.name}</strong> : {formatCurrency(topProduct.revenue)} de ventes sur {topProduct.quantity} unités.
            </p>
          </div>
        </div>
      )}
